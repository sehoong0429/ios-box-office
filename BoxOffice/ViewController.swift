...
final class ViewController: UIViewController {
    ...
    func fetchBoxOfficeData() {
        ...
        provider.performRequest(api: .boxOffice(date: yesterday)) { [weak self] requestResult in
...
//            content.text = "\(movie.rank)"
//            content.text = "\(movie.movieName)"
//
//            let formattedTodayAudience = self.decimal(Int(movie.audienceCount) ?? 0)
//            let formattedTotalAudience = self.decimal(Int(movie.audienceAcc) ?? 0)
//
//            content.secondaryText = "오늘 \(formattedTodayAudience) / 총 \(formattedTotalAudience)"
